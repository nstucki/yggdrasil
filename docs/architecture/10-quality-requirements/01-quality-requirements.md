...
| QS-13 | Configurability / Clarity | An operator reads the README looking for a way to set Eitri's model | The README names exactly one route (`agent.eitri.model` in `opencode.json`) and states that hand-editing the installed `eitri.md` is not supported, with its reasons | 1 documented route; 1 explicit "not a supported route" sentence naming `setup.sh` reversion and unverified host support; 0 sentences presenting the installed-file edit as an option | Doc test — `README.md:341-357` (as corrected by `fd0101a`) |
