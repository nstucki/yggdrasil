...
Error path: a second `BLOCKED` or a plan-level mismatch stops before persistence and surfaces the review — the run returns `review=<path> — BLOCKED` with `persisted=not-reached`, steps 6–8 never having been entered and no caller waiting to own the persistence decision (AC-12). *(Backported from the persisted 2026-09-19 delta.)*
