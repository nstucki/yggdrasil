...
Terms as the doctrine defines them; entries unchanged from the seed document are retained, the Pantheon entry is revised, and five entries are added.
...
| **Artifact** | A file, outside Yggdrasil Workspace and Yggdrasil Memory, that the task's implementation or design work creates or changes — including image files written by Eitri | `agents/brokk.md:75` (wording as harmonized by WP-5); `scripts/odin-generator/shared-body.template.md:46`; this revision |
...
| **Brief** | The exact text Odin passes to a Subtask at dispatch; retained for review dispatch | `agents/odin-autonomous.md:254` |
| **Image brief** | The fields of a brief addressed to Eitri: asset path, format, subject (required); dimensions, style, palette/design tokens, reference assets, constraints, revision-of (optional) | §5.1.2 of this revision |
...
| **Packaged workflow** | A trigger-gated, multi-dispatch pattern invoked whole via an `odin-*` skill | `agents/odin-autonomous.md:172-219` |
...
| **Capability inventory** | The generated skill listing built-in skills by role plus custom-granted tools, loaded once per session by Odin; six role sections after this revision | `agents/odin-autonomous.md:68`; `README.md:127`; §8.6 |
| **Skill** | A `SKILL.md` with frontmatter (`name`, `description`) and five required sections, loadable by the role its prefix names | `scripts/README.md:67-69` |
| **Designer** | The sixth specialist role: creates and revises image assets from a written brief; inventory section `### Designer`; role value `designer` in `custom-capabilities.yaml` | ADR-0011; §5.1.8 |
| **Eitri** | The Designer specialist — in myth the dwarven master-smith who, with his brother Brokkr, forged the gods' treasures; agent slug `eitri`, skill prefix `eitri-*`; its image model is selected only at the install site (`opencode.json` `agent.eitri.model`, ADR-0009 amended) | `README.md:65-67`; C11; §5.1.2 |
| **Image manifest** | The markdown Workfile `NN-image-<topic>.md` Eitri writes beside every image outcome: asset, format, dimensions, model, generation parameters, revision, brief compliance, open gaps | §5.1.2 |
| **Model override** | The install-site selection of Eitri's model: `agent.eitri.model` in the configuration home's `opencode.json` — the single supported route; hand-editing the installed `eitri.md` is not supported | ADR-0009 (amended); §8.10; `README.md:341-357` |
| **The Pantheon** | Odin plus the six specialists: Mimir (Researcher), Brokk (Implementer), Heimdall (Reviewer), Bragi (Communicator), Kvasir (Strategist), Eitri (Designer) | `agents/odin-autonomous.md:50-56`; `README.md:26-33`; this revision |
