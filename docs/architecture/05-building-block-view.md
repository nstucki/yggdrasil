...
- `brokk-architecture-persistence` (moved to `skills/architecture/brokk-architecture-persistence/SKILL.md` by WP-0; content unchanged): inputs unchanged (reviewed Workfile, ratification record, baseline, location, migration flag). Document mode supplies a ratification record listing the as-is ADR IDs ratified. **Open issue:** its index footer text "regenerated by the software engineering workflow's persistence step" (line 106) may want neutral wording — cosmetic, not in any write set.
