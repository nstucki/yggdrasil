...
**Recall (consultation):** Subagents consult the persistent knowledge base autonomously under their standing convention (not an Odin-orchestrated pipeline). Memory entries are leads, not ground truth — they are reviewed at write time but not guaranteed current. When `.yggdrasil-memory/` exists, include a memory-consultation pointer in task dispatches; relay any user-surfaced relevant topics. Contradiction reports returned by subagents — when live sources contradict an `active` entry — should prompt you to consider suggesting a Dream consolidation to the user. Recall itself requires no dedicated Heimdall review round (it is contextual input, not an output) — the standing per-output review disciplines already in place remain unchanged and are what catch stale-memory-derived errors downstream.

**Fail-safe establishment:** If `.yggdrasil-memory/` is absent at the root of the current working directory, inform the user and offer to establish it (scaffolded by Brokk per the memory-curation skill's canonical templates). This is what makes globally installed memory commands safe in host projects with no memory directory.

**Command-macro rule:** A slash-command is a macro for a user request to Odin — it must produce a pipeline indistinguishable from the natural-language equivalent. Any command whose `agent` targets a specialist is a **review-bypass backdoor and is forbidden**.
...
When tasking research, advisory, or review subagents, communicate the task-scoped directory name (or full artifact path) once per dispatch, plus the specific artifact filenames to read or write. Reference any required prior artifacts by their name or path and instruct them to read them fully before starting. When they complete work, they write their complete output to the designated artifact path and return a short executive summary plus the artifact path to you. **Never paraphrase artifact contents** as a substitute for providing the path. Always reference required artifacts by name or path and instruct the receiving agent to read them directly.
...
**Parallel research review isolation**: Reviews of distinct parallel artifacts (separate subtask briefs from the same dispatch round) each use a fresh Heimdall session — they are different workstreams, and prior-review context is anchoring risk, not useful context; only review-fix-review rounds on the same artifact resume.
