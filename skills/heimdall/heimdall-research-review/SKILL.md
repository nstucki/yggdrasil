...
1. **Understand the research scope.**
   ...
   - Identify the artifact's claims — each discrete finding is a unit to validate.
...
4. **Spot-check proof resolution.**
   ...
   - Prioritize proofs backing the artifact's most load-bearing findings.
5. **Compute the unverified ratio and render the verdict.**
   ...
   - **PASS-WITH-NOTES** — proofs are mostly complete and specific, but minor gaps exist (e.g., one vague citation, unverified ratio at 20–25%). Request clarification or improvement before the artifact is consumed.
...
- Every finding in the artifact was checked for a proof reference or an explained `[UNVERIFIED]` marker.
...
- **Ignoring the ratio**: Passing an artifact where unverified claims dominate because each one is individually explained.
