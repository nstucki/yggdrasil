...
2. **Architecture-context gate (conditional; skipped per the verdict).** Dispatch Mimir with `mimir-architecture-context`. Scope the brief by mode: in `document-existing` mode the scope is **the system as a whole** (or the named subsystem); in `decide-new` mode it is the objective's **structural footprint only — no task-scoped facts**: module boundaries, entry points, boundary interfaces, existing architecture documentation, and dependencies, but not the runner and its commands, not the conventions new code must imitate, not implementation-phase test infrastructure, and not what the touched paths do today. Those facts serve an implementation session, not an architecture document, and a document that absorbs them is stale as soon as the task ends. Require the output to be **fact-rich and framing-poor** — what is the case, not what should be built — with proofs per finding. Writes `NN-context-architecture-<area>.md`. Its standing review is a fresh Heimdall session with `heimdall-architecture-review` and `Focus: context`, writing `NN-review-context-architecture-<area>.md`.
...
6. **Ratification checkpoint (skipped when `persistence=deferred`).** Surface a readable summary: the mode and its source, the section scope **with the `Scope rationale:` recorded at step 4** — what the document deliberately leaves out and why, and which included sections were deliberately kept thin — the decisions awaiting ratification with their one-line rationales, the review verdict and any non-blocking notes, the open risks, the persistence location, and the remaining dispatch cost. The ratifier is ratifying the document's scope as much as its decisions, and a scope surfaced only as two lists of section numbers cannot be steered against. Whether to pause for steering or auto-proceed is governed by your Communication Policy; when auto-proceeding, ratify by adoption and let the summary ride the Deliverable disclosure.
...
- **The Deliverable discloses** the mode and its source, the section scope with its scope rationale, the decisions ratified, the persisted location, and the investigation gaps.
- **The context gate is scoped to structure, and the checkpoint shows the scope rationale.** A forward-looking context brief asks for the objective's structural footprint and declines task-scoped facts; the ratification summary carries the drafter's reason for every omitted and every deliberately minimized section. Both exist so the persisted document stays useful after this task is finished.
