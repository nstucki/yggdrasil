...
Inputs are the persistence manifest, the reviewed architecture Workfile **with its Appendix D layout map**, the ratification record the brief cites, the scaffold step's `Scaffold result` line, and the pinned baseline. There is no test-suite item here: a standalone architecture run has no suite to green.
...
- **(f) Decision records.** Each matches the reviewed record text, is numbered continuing the project's sequence from the reported `next-adr`, and sits in `09-architecture-decisions/` unless the project has its own decision-record directory, which keeps precedence. `Status: Accepted` appears only for the IDs the cited ratification record actually ratified — check decision by decision; an unratified `Accepted` is blocking, and so is a ratified record still reading `Proposed`. Log-to-file both ways, resolved **relative to `09-architecture-decisions/README.md`**: every row's link points at a file that exists whose own header matches the row's ID, title, status, and date, and every record file has a row. Rows are appended, never rewritten — a superseded record keeps its row and its file exactly as they were.
...
- **BLOCKED on:** a session that created structure, ran against an unscaffolded target, or ran against a top-index status the document scope contradicts; any deleted file under the target; a changed path the layout map does not account for; a topic document under the target that the map does not name; a persisted `_Not affected by this change_` marker; a promotion distance other than `depth − 1`; `Accepted` without a cited ratification, or a ratified record still reading `Proposed`; a decision log and record files that disagree; surviving `arc42 guidance §N` text with no notice beside it; a migration without cited user direction, or one performed by delete-and-recreate.
