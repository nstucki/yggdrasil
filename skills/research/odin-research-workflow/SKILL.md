...
1. **Decomposition (mandatory, advisory, no review).** Dispatch Kvasir with the `kvasir-research-decomposition` skill and the research topic. Kvasir writes a decomposition plan to a workspace artifact — N clusters with per-cluster investigation questions (N may be 1). This consultation is mandatory on every invoke, yet advisory: not independently reviewed (a standing exception to the Review Rules in your system prompt) and not a deliverable in the execution chain — defects surface through the per-stream reviews and the Final Review Gate. Fallback: if the dispatch fails (platform error, context limit), proceed with your own judgment — the mandate must not create a hard block on infrastructure failure.
...
4. **Parallel review.** For each Mimir artifact, dispatch a fresh Heimdall session (distinct-subtask isolation) with its `heimdall-research-review` skill. Parallelize across independent clusters. Each Heimdall writes `NN-review-cluster-<name>.md`.
5. **Synthesis.** Dispatch Mimir with all reviewed research artifacts and its `mimir-research-convention` skill to produce a unified synthesis — organized around the question, not around the sources, and naming its own boundaries: what was covered, what was not covered, and what remains uncertain. Writes `NN-synthesis.md`.
6. **Synthesis review.** Dispatch Heimdall with its `heimdall-research-review` skill to review the synthesis artifact before Bragi consumes it.
