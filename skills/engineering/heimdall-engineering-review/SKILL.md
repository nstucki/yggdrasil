...
| `integration` | a multi-package integration pass and the documents it persisted | no red suite and no unreviewed design reach the repository |
...
- **Header.** `Status: Proposed`, the document scope (`seed` or `update delta`), and the `Section scope: included=…, omitted=…` line. A document or record authored at `Status: Accepted` is blocking — ratification is not the author's to grant, and a self-ratified decision bypasses the checkpoint entirely.
...
- **Update-delta mode.** The document names the existing document it deltas and states which sections it replaces; it neither restates unaffected sections nor stands up a second, parallel document that disagrees with the first.
...
- **Persisted architecture document — merged, never overwritten.** In seed mode, the persisted document is the complete pruned document from the reviewed Workfile. In update-delta mode, only the sections the delta named are replaced, the §9 log rows are **appended** rather than rewritten, and every unaffected section is byte-identical to the baseline: diff the persisted file against the pinned baseline and confirm the changed hunks are exactly the named sections. Overwriting a pre-existing document with a delta is blocking — it deletes content whose removal nobody reviewed.
- **Decision records promoted correctly.** Each persisted record matches the reviewed record text, is numbered continuing the project's existing sequence, and sits in the project's existing decision-record directory when one exists. `Status: Accepted` appears only for decisions the ratification record the brief cites actually ratified — check it decision by decision. An unratified `Accepted` is blocking; so is a ratified decision persisted still reading `Proposed`.
- **Log-to-file consistency both ways.** Every §9 row resolves: its link points at a file that exists at the final relative path, and its ID, title, status, and date match that file's own header. Every persisted record file has a §9 row. A mismatch in either direction is blocking.
...
- **BLOCKED on:** an overwritten pre-existing document; `Accepted` without a cited ratification; a decision log and record files that disagree; a red suite or any regression against the baseline; a cross-package write-set violation.
